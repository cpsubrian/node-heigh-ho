    ...
    self.load(jobId, function (err, job) {
      ...
      trycatch(function () {
        self.handler(job, function (err, result) {
          if (err) return self._error(err);
          self.client.SMOVE(active, complete, jobId, function (err, moved) {
            if (err) return self._error(err);
            if (moved !== 1) {
              var error = new Error('Could not moved processed job from active to complete');
              error.job = job;
              return self._error(error);
            }
            self.emit('complete', job, result);
          });
        });
      }, function (err) {
        self._error(err);
