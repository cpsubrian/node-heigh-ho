 ...
 */
Queue.prototype.add = function (payload, options, done) {
  var self = this
    , job
    , cb
    , timeout
  ...
  // Wrap done callback in a timeout check.
  if (typeof done === 'function') {
    ...
    if (options.timeout || this.jobTimeout) {
      timeout = setTimeout(function () {
        cb(new Error('timeout waiting for job callback'));
      }, options.timeout || this.jobTimeout);
    }
  }
